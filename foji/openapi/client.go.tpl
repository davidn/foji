...
{{- define "clientAuth" -}}
    ...
    {{- $prefix := $.Params.GetWithDefault "TypePrefix" "" -}}
    {{- if eq $mode "param" -}}
        {{- if eq $kind "raw" -}}{{ $c }}Auth {{ $prefix }}ClientAuthenticator
        {{- else if eq $kind "basic" -}}{{ $c }}Auth {{ $prefix }}ClientBasicAuthenticator
        {{- else if eq $kind "cookie" -}}{{ $c }}Auth {{ $prefix }}ClientCookieAuthenticator
        {{- else if eq $kind "wrap" -}}{{ $c }}Auth {{ $prefix }}ClientWrappingAuthenticator
        {{- else -}}{{ $c }}Auth {{ $prefix }}ClientTokenAuthenticator
        {{- end -}}
...
{{- define "clientMethodSignature"}}
    ...
    {{- if $.OpSecuritySchemes $op }} user *{{ $.CheckPackage $.Params.Auth $package -}},{{ end }}
...
type (
	{{ $prefix }}ClientAuthenticator         = httputil.ClientAuthenticateFunc[*{{ $.CheckPackage $.Params.Auth $package }}]
	{{ $prefix }}ClientTokenAuthenticator    = httputil.ClientTokenAuthenticatorFunc[*{{ $.CheckPackage $.Params.Auth $package }}]
	{{ $prefix }}ClientBasicAuthenticator    = httputil.ClientBasicAuthenticatorFunc[*{{ $.CheckPackage $.Params.Auth $package }}]
	{{ $prefix }}ClientCookieAuthenticator   = httputil.ClientCookieAuthenticatorFunc[*{{ $.CheckPackage $.Params.Auth $package }}]
	{{ $prefix }}ClientWrappingAuthenticator = httputil.ClientWrappingAuthenticatorFunc[*{{ $.CheckPackage $.Params.Auth $package }}]

{{- if .HasComplexAuth }}
	{{ $prefix }}ClientSecurityGroup  = httputil.ClientSecurityGroup[*{{ $.CheckPackage $.Params.Auth $package }}]
	{{ $prefix }}ClientSecurityGroups = httputil.ClientSecurityGroups[*{{ $.CheckPackage $.Params.Auth $package }}]
{{- end }}
)

{{ end -}}

type {{ $prefix }}Methods interface {
...
type {{ $prefix }}Client struct {
	baseURL    string
	httpClient *http.Client
{{- range $security, $value := .API.Components.SecuritySchemes }}
	{{ camel $security }}Auth {{ $prefix }}ClientAuthenticator
{{- end }}
{{- range $name, $path := .API.Paths.Map }}
    {{- range $verb, $op := $path.Operations }}
        {{- if not ($.IsSimpleAuth $op) }}
	{{ camel $op.OperationID}}Security {{ $prefix }}ClientSecurityGroups
        {{- end}}
    {{- end}}
{{- end}}
}

func New{{ $prefix }}Client(baseURL string, httpClient *http.Client
{{- range $security, $value := .API.Components.SecuritySchemes }}, {{ template "clientAuth" ($.WithParams "scheme" $security "mode" "param") }}{{- end }}) *{{ $prefix }}Client {
	c := &{{ $prefix }}Client{
		baseURL:    baseURL,
...
{{- range $name, $path := .API.Paths.Map }}
	...
	c.{{ camel $op.OperationID}}Security = {{ $prefix }}ClientSecurityGroups{
            {{- range $securityGroup := $.OpSecurity $op }}
		{{ $prefix }}ClientSecurityGroup{
...
func (c *{{ $prefix }}Client) {{ pascal $op.OperationID }}(ctx context.Context,
    {{- template "clientMethodSignature" ($.WithParams "op" $op "package" $package "path" $path) }} {
	u := c.baseURL + "{{ $name }}"
        {{- $hasQuery := false }}
